...
- Piper: run `tools/voice_engines/install_piper.ps1`, then set `JARVIS_PIPER_ENABLED=true`, `JARVIS_PIPER_MODEL_PATH=models/piper/ru_RU.onnx`, and `JARVIS_PIPER_CONFIG_PATH=models/piper/ru_RU.onnx.json`.
- XTTS v2: set `JARVIS_XTTS_ENABLED=true` and run a local API at `JARVIS_XTTS_API_URL`, default `http://127.0.0.1:8020`.
- GPT-SoVITS: run `tools/voice_engines/install_gpt_sovits.ps1` for manual external setup instructions, then set `JARVIS_GPT_SOVITS_ENABLED=true` and run the API at `JARVIS_GPT_SOVITS_API_URL`, default `http://127.0.0.1:9880`.
...
Full local voice notes live in `docs/local_voice_engines.md`.
